...
export function mountLab(host, createExperiment, options={}) {
  ...
  const animate=now=>{if(disposed)return;raf=requestAnimationFrame(animate);const dt=Math.max(0,Math.min(1/30,(now-last)/1000));last=now;if(!visible||document.hidden||contextLost)return;if(dt>0){elapsed+=dt;for(const frame of frames)frame(dt,elapsed);}if(controls.update())dirty=true;if(dirty){renderer.render(scene,camera);dirty=false;}};raf=requestAnimationFrame(animate);
