...
export function mountLab(host, createExperiment, options={}) {
  ...
  const listeners=[],frames=new Set(),downloads=new Set();let dirty=true,disposed=false,visible=true,elapsed=0,last=performance.now(),raf;
  ...
  function setStatus(text){status.textContent=text;invalidate();}
  function fit(object=root){
    ...
    const center=box.getCenter(new THREE.Vector3()),direction=new THREE.Vector3(1,.72,1.1).normalize(),right=new THREE.Vector3().crossVectors(new THREE.Vector3(0,1,0),direction).normalize(),up=new THREE.Vector3().crossVectors(direction,right).normalize();
    const tanV=Math.tan(THREE.MathUtils.degToRad(camera.fov/2)),tanH=tanV*camera.aspect;let d=1;
    for(const x of [box.min.x,box.max.x])for(const y of [box.min.y,box.max.y])for(const z of [box.min.z,box.max.z]){const v=new THREE.Vector3(x,y,z).sub(center),depth=v.dot(direction);d=Math.max(d,depth+Math.abs(v.dot(right))/tanH,depth+Math.abs(v.dot(up))/tanV);}
    d*=1.12;controls.target.copy(center);camera.position.copy(center).addScaledVector(direction,d);camera.near=Math.max(.001,d/1000);camera.far=Math.max(100,d*40);controls.maxDistance=Math.max(30,d*5);camera.updateProjectionMatrix();controls.update();invalidate();
  }
  ...
  function resize(){const width=viewport.clientWidth,height=viewport.clientHeight;if(!width||!height)return;renderer.setSize(width,height,false);camera.aspect=width/height;camera.updateProjectionMatrix();invalidate();}
  ...
  const animate=now=>{if(disposed)return;raf=requestAnimationFrame(animate);const dt=Math.min(1/30,(now-last)/1000);last=now;if(!visible||document.hidden)return;elapsed+=dt;for(const frame of frames)frame(dt,elapsed);if(controls.update())dirty=true;if(dirty){renderer.render(scene,camera);dirty=false;}};raf=requestAnimationFrame(animate);
