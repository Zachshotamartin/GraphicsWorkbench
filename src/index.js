...
export function mountLab(host, createExperiment, options={}) {
  ...
  const workspace=el('div','graphics-workbench__workspace'),panel=el('div','graphics-workbench__panels'),view=el('div','graphics-workbench__view'),viewport=el('div','graphics-workbench__viewport'),toolbar=el('div','graphics-workbench__toolbar'),status=el('p','graphics-workbench__status','Preparing geometry…');
  ...
  const sessions=new Map(),listeners=[],downloads=new Set(),pointers=new Set();
  let active,disposed=false,visible=true,contextLost=false,dirty=true,last=performance.now(),raf;
  ...
  function showStatus(){status.textContent=contextLost?'Graphics paused. Waiting for the browser to restore the 3D view…':active?.statusText||'Preparing geometry…';}
  function setStatus(text){if(active)active.statusText=text;showStatus();invalidate();}
  ...
  listen(canvas,'pointerdown',event=>pointers.add(event.pointerId),{capture:true});
  listen(window,'pointerup',event=>pointers.delete(event.pointerId));
  listen(window,'pointercancel',event=>pointers.delete(event.pointerId));
  listen(canvas,'webglcontextlost',event=>{event.preventDefault();contextLost=true;showStatus();});
  listen(canvas,'webglcontextrestored',()=>{contextLost=false;last=performance.now();showStatus();invalidate();});

  function makeSession(key,create){
    const session={key,panel:el('div','graphics-workbench__controls'),frames:new Set(),listeners:[],elapsed:0,statusText:'Preparing geometry…',cursor:'',experiment:null};
    sessions.set(key,session);active=session;panel.append(session.panel);
    const panelNode=session.panel,sessionPrefix=prefix+'-'+sessions.size;
  ...
  Object.assign(session,{scene,root,camera,controls});
  const frames=session.frames;
  const invalidate=()=>{if(active===session)dirty=true;};
  function listen(target,type,fn,config){
    // UI input belongs only to the visible tool; workers may finish in the background.
    const isInput=target===window||target instanceof Node;
    const handler=(...args)=>{if(!disposed&&(!isInput||active===session))return fn(...args);};
    target.addEventListener(type,handler,config);session.listeners.push(()=>target.removeEventListener(type,handler,config));return handler;
  }
  controls.addEventListener('change',invalidate);
  const id=()=>`${sessionPrefix}-${panelNode.querySelectorAll('input,select,button').length}`;
  function field(label){const wrap=el('label','graphics-workbench__field'),caption=el('span','graphics-workbench__label',label);wrap.append(caption);panelNode.append(wrap);return {wrap,caption};}
  ...
  const ui={
    button(label,fn,{primary=false}={}){const b=el('button',primary?'is-primary':'',label);b.type='button';listen(b,'click',action(fn));panelNode.append(b);return b;},
    ...
    note(text){const p=el('p','graphics-workbench__note',text);panelNode.append(p);return p;},
    section(text){const h=el('h3','graphics-workbench__section',text);panelNode.append(h);return h;},
  };
  function setStatus(text){session.statusText=text;if(active===session)showStatus();invalidate();}
  ...
  session.resize=()=>{
    ...
    camera.aspect=aspect;camera.updateProjectionMatrix();controls.update();invalidate();
  };
  session.resize();
  session.experiment=create(ctx)||{};fit();
  return session;
  }
  ...
  function suspend(session){
    releaseInput();session.experiment?.deactivate?.();
    session.cursor=canvas.style.cursor;session.controls.disconnect();session.panel.remove();
  }
  function resume(session){
    active=session;panel.append(session.panel);session.controls.connect(canvas);canvas.style.cursor=session.cursor;
    session.experiment?.activate?.();session.resize();showStatus();last=performance.now();invalidate();
  }
  function switchExperiment(key,create){
    if(disposed)throw new Error('This workbench has been disposed.');
    if(active?.key===key)return active.ctx;
    const previous=active;
    if(previous)suspend(previous);
    if(sessions.has(key)){resume(sessions.get(key));return active.ctx;}
    canvas.style.cursor='';
    try{const session=makeSession(key,create);showStatus();last=performance.now();invalidate();return session.ctx;}
    catch(error){
      const failed=sessions.get(key);if(failed)destroySession(failed);sessions.delete(key);
      if(previous)resume(previous);else active=null;
      throw error;
    }
  }
  function toolButton(label,fn){const b=el('button','',label);b.type='button';listen(b,'click',()=>{try{fn();}catch(error){setStatus(error.message);}});toolbar.append(b);}
  const rotate=amount=>{if(!active)return;const {camera,controls}=active;const offset=camera.position.clone().sub(controls.target);offset.applyAxisAngle(new THREE.Vector3(0,1,0),amount);camera.position.copy(controls.target).add(offset);controls.update();invalidate();};
  toolButton('Rotate left',()=>rotate(-Math.PI/12));toolButton('Rotate right',()=>rotate(Math.PI/12));
  listen(canvas,'keydown',event=>{if(event.defaultPrevented)return;if(event.key==='ArrowLeft'||event.key==='ArrowRight'){event.preventDefault();rotate(event.key==='ArrowLeft'?-Math.PI/12:Math.PI/12);}else if(event.key==='Home'){event.preventDefault();active?.fit();}});
  toolButton('Reset view',()=>active?.fit());toolButton('Save PNG',()=>{if(active){renderer.render(active.scene,active.camera);canvas.toBlob(blob=>blob&&download('experiment.png',blob,'image/png'));}});
  ...
  const animate=now=>{
    ...
    const dt=Math.max(0,Math.min(1/30,(now-last)/1000));last=now;
    if(!active||!visible||document.hidden||contextLost)return;
    if(dt>0){active.elapsed+=dt;for(const frame of active.frames)frame(dt,active.elapsed);}
    if(active.controls.update())dirty=true;
    if(dirty){renderer.render(active.scene,active.camera);dirty=false;}
  };raf=requestAnimationFrame(animate);
  function destroySession(session){
    session.experiment?.dispose?.();session.listeners.forEach(off=>off());session.frames.clear();session.controls?.dispose();session.panel.remove();
    const geometries=new Set(),materials=new Set(),textures=new Set();
    session.scene?.traverse(o=>{if(o.geometry)geometries.add(o.geometry);for(const m of Array.isArray(o.material)?o.material:o.material?[o.material]:[]){materials.add(m);for(const v of Object.values(m))if(v?.isTexture)textures.add(v);}});
    geometries.forEach(g=>g.dispose());materials.forEach(m=>m.dispose());textures.forEach(t=>t.dispose());
  }
  function dispose(){if(disposed)return;releaseInput();disposed=true;cancelAnimationFrame(raf);observer.disconnect();intersection.disconnect();sessions.forEach(destroySession);sessions.clear();listeners.forEach(off=>off());renderer.dispose();renderer.forceContextLoss();downloads.forEach(URL.revokeObjectURL);host.replaceChildren();}
  return {dispose,switchExperiment,get ctx(){return active?.ctx;},get activeKey(){return active?.key;}};
}
